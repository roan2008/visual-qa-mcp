...
- `chart-v2` has a frozen, checksum-manifested 18-case covering-array dataset (`datasets/charts/chart-v2-covering-v1`, `generate-chart-covering-dataset` CLI command, validated via the existing generic `run-validation --dataset ...` command) implementing a formal input model: Matrix A (in-universe presentation x defect, exhaustively enumerated, 12 cases) and Set B (any out-of-universe axis flipped x defect, always expected `needs_review`, 6 cases, proves masking). See `wiki/impl-chart-v2-covering-array-input-model.md`. It does not cover layout-mismatch (not reproducible with the current renderer) or continuous-nuisance axes, and is chart-v2-only.
