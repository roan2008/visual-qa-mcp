...
def test_mcp_verify_coordinate_returns_schema_valid_payload(tmp_path: Path) -> None:
    dataset_root = tmp_path / "coordinate-graph-v1"
    build_coordinate_dataset(dataset_root)
    case = next(
        case for case in discover_coordinate_cases(dataset_root) if case.case_id == "mutated-03"
    )
    claim_schema = load_schema(ROOT / "specs" / "claim-graph.schema.json")
    evidence_schema = load_schema(ROOT / "specs" / "coordinate-evidence-graph.schema.json")
    findings_schema = load_schema(ROOT / "specs" / "findings.schema.json")
    primitive_schema = load_schema(ROOT / "specs" / "primitive-evidence-graph.schema.json")
    server = create_server()
